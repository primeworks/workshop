...
class VoguePayServer2Server(VoguePayCommon):
    ...
    def test_00_tx_management(self):
        ...
        res = self.payment_acquirer._voguepay_s2s_get_access_token(cr, uid, [self.voguepay_id], context=context)
        self.assertTrue(res[self.voguepay_id] is not False, 'voguepay: did not generate access token')
        ...
        self.assertTrue(tx.voguepay_txn_id is not False, 'voguepay: txn_id should have been set after s2s request')

        self.payment_transaction.write(cr, uid, tx_id, {'voguepay_txn_id': False}, context=context)
...
class VoguePayForm(VoguePayCommon):
    ...
    def test_10_voguepay_form_render(self):
        cr, uid, context = self.cr, self.uid, {}
        # be sure not to do stupid things
        self.payment_acquirer.write(cr, uid, self.voguepay_id, {'fees_active': False}, context)
        voguepay = self.payment_acquirer.browse(cr, uid, self.voguepay_id, context)
        self.assertEqual(voguepay.environment, 'test', 'test without test environment')
        ...
        form_values = {
            'cmd': '_xclick',
            'business': 'tde+voguepay-facilitator@openerp.com',
            'item_name': 'test_ref0',
        ...
        self.assertEqual(tree.get('action'), 'https://www.sandbox.voguepay.com/cgi-bin/webscr', 'voguepay: wrong form POST url')
    ...
    def test_11_voguepay_form_with_fees(self):
        cr, uid, context = self.cr, self.uid, {}
        # be sure not to do stupid things
        voguepay = self.payment_acquirer.browse(self.cr, self.uid, self.voguepay_id, None)
        self.assertEqual(voguepay.environment, 'test', 'test without test environment')
        ...
        self.payment_acquirer.write(cr, uid, self.voguepay_id, {
            'fees_active': True,
            'fees_dom_fixed': 1.0,
            'fees_dom_var': 0.35,
            'fees_int_fixed': 1.5,
        ...
        for form_input in tree.input:
            if form_input.get('name') in ['handling']:
                handling_found = True
                self.assertEqual(form_input.get('value'), '1.57', 'voguepay: wrong computed fees')
        self.assertTrue(handling_found, 'voguepay: fees_active did not add handling input in rendered form')
    ...
    def test_20_voguepay_form_management(self):
        ...
        voguepay_post_data = {
            ...
            'receiver_email': u'tde+voguepay-facilitator@openerp.com',
            ...
            'business': u'tde+voguepay-facilitator@openerp.com',
            'test_ipn': u'1',
        ...
        # check
        tx = self.payment_transaction.browse(cr, uid, tx_id, context=context)
        self.assertEqual(tx.state, 'pending', 'voguepay: wrong state after receiving a valid pending notification')
        self.assertEqual(tx.state_message, 'multi_currency', 'voguepay: wrong state message after receiving a valid pending notification')
        self.assertEqual(tx.voguepay_txn_id, '08D73520KX778924N', 'voguepay: wrong txn_id after receiving a valid pending notification')
        self.assertFalse(tx.date_validate, 'voguepay: validation date should not be updated whenr receiving pending notification')

        # update tx
        self.payment_transaction.write(cr, uid, [tx_id], {
            'state': 'draft',
            'voguepay_txn_id': False,
        }, context=context)
        # update notification from voguepay
        voguepay_post_data['payment_status'] = 'Completed'
        # validate it
        self.payment_transaction.form_feedback(cr, uid, voguepay_post_data, 'voguepay', context=context)
        # check
        tx = self.payment_transaction.browse(cr, uid, tx_id, context=context)
        self.assertEqual(tx.state, 'done', 'voguepay: wrong state after receiving a valid pending notification')
        self.assertEqual(tx.voguepay_txn_id, '08D73520KX778924N', 'voguepay: wrong txn_id after receiving a valid pending notification')
        self.assertEqual(tx.date_validate, '2013-11-18 03:21:19', 'voguepay: wrong validation date')
